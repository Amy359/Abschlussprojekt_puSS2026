...
with tab_recovery:
    ...
    m3.caption(f"KH: {df_r_ath['Kohlenhydrate_g'].mean():.0f}g | P: {df_r_ath['Proteine_g'].mean():.0f}g | F: {df_r_ath['Fette_g'].mean():.0f}g")

    if not df_r_ath.empty:
        col_g1, col_g2 = st.columns(2)
        col_g1.plotly_chart(px.line(df_r_ath, x="Datum", y="Ruhepuls_bpm", title="Trend Ruhepuls (bpm)", color_discrete_sequence=['red']), use_container_width=True)
        col_g2.plotly_chart(px.line(df_r_ath, x="Datum", y="HRV_ms", title="Trend HRV (ms)", color_discrete_sequence=['green']), use_container_width=True)