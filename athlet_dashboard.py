...
def load_data():
    ...
    if os.path.exists("data/triathlon_training.csv"):
        ...
        df_t = pd.DataFrame(
            columns=["Datum", "Athlet", "Aktivität", "Dauer_min", "Distanz_km", "Max_Herzfrequenz", "Schmerzen"]
        )
...
def show_calendar(df_user_t):
    ...
    for _, row in df_user_t.iterrows():
        color = "#10b981" if row["Aktivität"] != "Ruhetag" else "#fca5a5"
        calendar_events.append(
            {
                "title": f"{row['Aktivität']} ({row['Distanz_km']}km)",
...
def athlet_dashboard(person):
    ...
    aktiver_athlet = person.get_vollname()
    ...
    menu = st.sidebar.radio(
        "Navigation", ["📅 Mein Kalender", "🏋️ Training eingeben", "🛌 Regeneration eingeben", "📊 Meine Auswertung"]
    )
                    ...
                    [
                        {
                            ...
                            "Athlet": aktiver_athlet,
                            "Aktivität": aktivitaet,
                            "Dauer_min": dauer,
                            "Distanz_km": distanz,
                            "Max_Herzfrequenz": puls,
                            "Schmerzen": schmerzen,
                        }
                    ]
                )
    ...
    # 4. AUSWERTUNG
        ...
        if df_t_ath.empty:
            ...
            with t1:
                kw_data = df_t_ath.groupby(["KW", "Aktivität"])["Distanz_km"].sum().reset_index()
                st.plotly_chart(px.bar(kw_data, x="KW", y="Distanz_km", color="Aktivität"), use_container_width=True)
            with t2:
